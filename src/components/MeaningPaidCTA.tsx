...
export function MeaningPaidCTA({ number }: MeaningPaidCTAProps) {
  ...
  const canCheckout = Boolean(focus && feeling && timeHorizon);
  ...
  const startCheckout = async () => {
    ...
    try {
      trackEvent('cta_click', {
        ...
        metadata: { label: 'Meaning PDF CTA', number },
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-zinc-500 text-sm">
        Required: choose focus, feeling, and time horizon. We build your PDF on Life Path {number} plus your inputs.
      </p>
