...
export async function generateReportPdf(payload: ReportPayload) {
  ...
  const chunks: Buffer[] = [];
  const filename =
    payload.product === 'quick_report' ? 'quick-number-report.pdf' : 'numerology-blueprint.pdf';

  doc.on('data', chunk => chunks.push(chunk as Buffer));

  if (payload.product === 'quick_report') {
    const num = payload.numbers?.[0];
    addHeader(
      doc,
      'Quick Number Report',
      num !== undefined ? `Angel Number ${num} • ${new Date().toLocaleDateString()}` : new Date().toLocaleDateString()
    );
    if (num !== undefined) {
      ...
      addSection(doc, {
        ...
        body: 'Use this number as your anchor for the next 7 days. Notice when it appears and take one small aligned action. Want a full personalized blueprint? Visit spiritnumeral.com/quiz.',
      });
    } else {
      addSection(doc, {
        title: 'Your Number',
        body: 'No number was provided. Visit spiritnumeral.com to get your quick report with a number.',
      });
    }
    addFooter(doc);
    doc.end();
